...
def create_github_meta(new_meta_df: pd.DataFrame, old_meta_filepath: str, meta_cols: list):
    ...
    ## TODO: rename ccbb cols to github equivalent
...
def create_gisaid_meta(new_meta_df: pd.DataFrame, meta_cols: list):
    ...
    ## TODO: rename ccbb cols to gisaid equivalent
...
def get_ids(filepaths: list) -> list:
    # TODO: get IDs from CCBB consensus filenames
    raise NotImplementedError
...
if __name__=="__main__":
    ...
    git_meta_cols = ["ID", "collection_date", "location", "percent_coverage_cds", 
                     "Coverage", "authors", "originating_lab", "fasta_hdr"]
    ...
    parser.add_argument("-a", "--analysis-folder",
                        type=str,
                        required=True,
                        help="Path to the analysis folder that contains all sequencing files")
    ...
    # cov_df.loc[:,'sample_id'] = cov_df['SAMPLE'].apply(lambda x: x.split('_')[0])
    ...
    low_depth_samples = ans.loc[(ans["percent_coverage_cds"] >= min_coverage) 
    ...
    if not dry_run:
        ...
        # TODO: change to align_reference()
        ...
        low_coverage_samples.to_csv(out_dir/'low_coverage_samples.csv', index=False)
        low_depth_samples.to_csv(out_dir/'low_depth_samples.csv', index=False)
    else:
        sus_ids = []
    ...
    with open("{}/data_release.log".format(out_dir), 'w') as f:
        ...
        f.write(f'{low_depth_samples.shape[0]} samples were found to have coverage above {min_coverage}% BUT depth below {min_depth}\n')
